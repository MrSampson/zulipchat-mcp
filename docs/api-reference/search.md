...
- When any of `last_hours`/`last_days`/`after_time`/`before_time` is set, matching messages are fetched by walking the anchor backward page by page - toward the cutoff if one of the first three is set, or until enough `before_time`-matching messages are found otherwise - so a `before_time` upper bound further back than one page's worth of newer traffic still gets found. The response's `window_complete` flag is `False` if a hard page cap was hit before the window was confirmed covered, signaling the result may be incomplete.
