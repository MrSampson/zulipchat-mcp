...
- `advanced_search` aggregates across messages/users/streams and can return basic counts. It validates `limit` the same way `search_messages` does, at the top level - an out-of-range `limit` returns the top-level `INVALID_LIMIT` error directly, rather than a top-level success with the error buried in `results["messages"]`.
